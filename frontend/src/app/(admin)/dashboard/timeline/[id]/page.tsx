...
export default function EditMilestonePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { register, handleSubmit, setValue, formState: { errors } } = useForm<MilestoneFormData>();
    ...
    const fetchMilestone = async () => {
        try {
            setIsLoading(true);
            const data = await apiRequest(`/timeline/${id}`);
            if (data) {
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-slate-900 font-display">
                        Edit <span className="text-blue-600">Milestone</span>
                    </h1>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-1.5">
                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                                    Year <span className="text-blue-500 font-bold">*</span>
                                </label>
                                <input
                                    type="text"
                                    {...register('year', { required: 'Year is required' })}
                                    className={`w-full bg-slate-50 border-2 rounded-2xl px-5 py-4 text-sm font-medium transition-all ${errors.year
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    {...register('icon')}
                                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 text-sm font-medium transition-all focus:border-blue-500 focus:ring-blue-500/10"
                                    placeholder="e.g. flag-checkered"
                                />
                            </div>
